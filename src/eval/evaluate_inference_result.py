...
# python -m src.eval.evaluate_inference_result --model gpt-4o-mini-2024-07-18 --dataset ./src/data/test_data_wo_korean.jsonl --num_workers 64 --eval_localization trueion true
...
Please evaluate whether the error location indicated in the [AI's Generated Error Location] is correct. The generated error location must be a specific mathematical expression in the [Student's Solution]. 
...
def evaluate_localization(infernece_result: list, batch_size: int=64):
    ...
    for idx, _ in enumerate(tqdm(infernece_result[::batch_size])):
        ...
        for item in infernece_result[_from:_to]:
            ...
            student_image = images[1]
            
            # Get the file name of the image
            question_image_name = os.path.basename(question_image)
            student_image_name = os.path.basename(student_image)
            question_id = question_image_name[:-4]
            student_id = student_image_name[:-5]
            q_text = qa_trans[qa_trans["id"] == question_id]["question_en"].values[0]
            r_text = qa_trans[qa_trans["id"] == question_id]["answer_en"].values[0]
            # print("Question:", q_text)
            # print("Reference answer:", r_text)
            with open(f"/path/to/aihub/dataset/110.수학_과목_자동_풀이_데이터/3.개방데이터/1.데이터/Labels/valid/{student_id}.json", "r", encoding="utf-8-sig") as f:
                student = json.load(f)
            # print("Student answer: ", student["explanation_info"][0]["explanation_text"])
            questions.append(q_text)
            references.append(r_text)
            students.append(student["explanation_info"][0]["explanation_text"])
...
if __name__ == "__main__":
    ...
    parser.add_argument("--port", type=int, default=8010, help="port number for inference server")
    ...
    if args.eval_localization:
        ...
        localize_accuracy, localize_precision, localize_recall, localize_f1, y_pred, y_true = evaluate_localization(inference_result, args.num_workers)
